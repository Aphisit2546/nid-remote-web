...
import { Menu, ChevronLeft, Search, Printer } from 'lucide-react';
...
// Mock data - ว่างเปล่าจนกว่าจะดึงข้อมูลจาก API
...
export default function DoorHistoryPage() {
    ...
    const [statusFilters, setStatusFilters] = useState<{ open: boolean; stop: boolean; close: boolean }>({
        open: false,
        stop: false,
        close: false
    });
        ...
        <div style={{
            ...
            <main style={{
                ...
                <div style={{
                    ...
                    {/* Status Filter - 3 options (Checkbox) */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '0.75rem', color: '#374151' }}>สถานะ</span>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.7rem', color: '#374151' }}>
                            <input
                                type="checkbox"
                                checked={statusFilters.open}
                                onChange={(e) => setStatusFilters(prev => ({ ...prev, open: e.target.checked }))}
                        ...
                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.7rem', color: '#374151' }}>
                            <input
                                type="checkbox"
                                checked={statusFilters.stop}
                                onChange={(e) => setStatusFilters(prev => ({ ...prev, stop: e.target.checked }))}
                                style={{ width: '0.75rem', height: '0.75rem' }}
                                suppressHydrationWarning
                            />
                            หยุดประตู
                        ...
                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.7rem', color: '#374151' }}>
                            <input
                                type="checkbox"
                                checked={statusFilters.close}
                                onChange={(e) => setStatusFilters(prev => ({ ...prev, close: e.target.checked }))}
                                style={{ width: '0.75rem', height: '0.75rem' }}
                                suppressHydrationWarning
                            />
                            ปิดประตู
